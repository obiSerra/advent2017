...
(def real-input "o inc 394 if tcy >= -3
s inc -204 if s <= -5
keq inc -34 if keq > -6
ip inc 762 if ip == 4
zr inc -644 if ip >= 6
...
(defmacro do-fn [pre x y]
  `(~pre ~x ~y)
  )

(defn calc-line [acc r]
  (let [ar (clojure.string/split r #" ")
        k (-> ar first str keyword)
        v (get-v k acc)
        add-fn (get-fn (second ar))
        op (-> ar rest rest first str read-string)
        c (-> ar rest rest rest rest rest first str)
        x (-> ar rest rest rest rest first str)
        xv (get-v (keyword x) acc)
        y (-> ar last str read-string)
        tst (do-fn (get-pred c) xv  y)
        new-v (do-fn add-fn v op)
        rg (:register acc [])]   
    
    (if tst
      (assoc acc k new-v :register (conj rg new-v))
      acc
      )    
    )
  )

(defn process-register [input]
  (->> input split-input (reduce calc-line {}))
  )

(defn solve-quiz [input]
  (->> input process-register (#(dissoc % :register)) vals (apply max))
  )

(defn solve-quiz-2 [input]
  (->> input process-register :register (apply max))
  )

(is (= 1 (solve-quiz test-input)))
(is (= 10 (solve-quiz-2 test-input)))

;; (solve-quiz test-input)
;; (solve-quiz-2 real-input)
